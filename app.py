...
INDUSTRY_MULTIPLIERS = {
    ...
    "Chemicals and petrochemicals manufacturing": [1.10, 1.05, 1.00, 0.95, 1.15, 0.95],
...
.methodology-copy p.methodology-footer { margin-top:30px !important; padding-top:14px; border-top:1px solid rgba(43,86,136,.12); color:var(--ink-muted); font-size:11px !important; line-height:1.45; letter-spacing:.015em; text-transform:uppercase; }
@media (max-width:760px) { .validation-grid { grid-template-columns:1fr; } .validation-value { text-align:left; margin-bottom:7px; } .validation-box { width:100%; } .methodology-copy p.methodology-footer { font-size:11px !important; } .market-development-grid { grid-template-columns:1fr; } }
...
.st-key-survey_submit_row .stButton > button:hover,
.st-key-survey_submit_row .stButton > button:focus-visible,
.st-key-survey_submit_row .stButton > button:active {
    border-color: #1f4775 !important;
    background: #1f4775 !important;
    color: #ffffff !important;
}
.st-key-survey_submit_row .stButton > button:disabled {
...
/* Full-screen mobile blocker for analytical pages. The survey route does not render it. */
.mobile-desktop-notice {
    display: none;
}
.mobile-desktop-notice.force-show {
...
@media (max-width: 900px) {
    ...
    .mobile-desktop-notice-inner {
        ...
        box-shadow: 0 12px 36px rgba(43,86,136,.10) !important;
    }
...
def load_index_data(file_path: str, file_version: str) -> pd.DataFrame:
    ...
    if "Operating coal capacity (MW)" in frame.columns:
        numeric_columns.append("Operating coal capacity (MW)")
    ...
    if "Operating coal capacity (MW)" not in frame.columns and "Screening" in workbook.sheetnames:
    ...
    if "Operating coal capacity (MW)" not in frame.columns:
        ...
        frame["Operating coal capacity (MW)"] = pd.to_numeric(
            frame["Operating coal capacity (MW)"], errors="coerce"
        )
...
@st.cache_data(show_spinner=False)
def load_market_overlay(file_path: str, file_version: str) -> pd.DataFrame:
...
def render_mobile_desktop_notice(*, force_show: bool = False) -> None:
    """Cover analytical pages on phones while leaving the direct survey mobile-friendly."""
    force_class = " force-show" if force_show else ""
    st.markdown(
        f"""
        <div class="mobile-desktop-notice{force_class}" role="alert">
          <div class="mobile-desktop-notice-inner">
            <strong>This app can only be viewed on desktop.</strong>
...
def ranking_figure(results: pd.DataFrame, show_all: bool, mode: str) -> go.Figure:
    ...
    for _, row in displayed.iterrows():
        ...
        capacity_value = pd.to_numeric(row.get("Operating coal capacity (MW)"), errors="coerce")
        capacity_text = "Not available" if pd.isna(capacity_value) else f"{float(capacity_value):,.0f} MW"
        customdata.append(
            [int(row["Rank"]), change_text, int(row.get("Base rank", row["Rank"])), capacity_text]
        )
...
def validation_request_is_mobile() -> bool:
    """Return True for phone and tablet user agents when headers are available."""
    try:
...
SURVEY_MORE_INFO_BODY = """
<p><strong>More about coal-to-clean projects and transition credits</strong></p>
    ...
    with st.container(key="validation_section"):
        ...
        st.markdown(
            ...
            <div class="survey-intro">
              ...
              <div class="survey-time">All survey questions are required unless labelled optional. Estimated completion time: 8–10 minutes</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        with st.container(key="validation_form_shell"):
            ...
            if validation_request_is_mobile():
                # Phones and tablets remain on the mobile-friendly survey page.
            ...
            # Desktop respondents return to the main index after submission.
            st.session_state.survey_entry_locked = False
            st.session_state.survey_success_pending = True
            if SURVEY_ROUTE_PARAM in st.query_params:
                del st.query_params[SURVEY_ROUTE_PARAM]
            st.query_params["view"] = "overall"
            if "result" in st.query_params:
...
try:
    # Hash the workbook contents so replacing a file under the same name
    # invalidates every cached data loader immediately.
    data_file_version = hashlib.sha256(DATA_FILE.read_bytes()).hexdigest()
    index_data = load_index_data(str(DATA_FILE), data_file_version)
    overlay_data = load_market_overlay(str(DATA_FILE), data_file_version)
...
if view == "validation":
    ...
    if validation_request_is_mobile():
        ...
        render_mobile_desktop_notice(force_show=True)
        st.stop()
    render_sidebar(view)
    render_header()
