...
VALIDATION_SURVEY_VERSION = "2026-07-29-v1"
VALIDATION_SCALE = {
    1: "Not important",
...
VALIDATION_FACTORS: list[dict[str, Any]] = [
    {
        ...
        "description": "Solar and/or wind generation has grown steadily in the country.",
        "construct_id": "C05B",
    ...
    },
    {
        ...
        "description": "The country has a clear and credible net-zero target.",
        "construct_id": "C03",
    ...
    },
    {
        ...
        "description": "The country has set a clear target year for ending unabated coal power.",
        "construct_id": "C03",
        "construct": "Policy durability and implementation credibility",
        "indicator_id": "P2-I8",
    },
    ...
    },
    {
        ...
        "description": "The country has an operating carbon tax or emissions trading system covering major emitters.",
        "construct_id": "C07",
    ...
    },
    {
        ...
        "description": "The country has hosted carbon-credit projects – such as REDD+, methane capture, clean cooking or biochar – that have progressed to credit issuance or buyer use.",
    ...
    },
    {
        ...
        "description": "The country has clear procedures for approving and accounting for carbon credits transferred internationally.",
        "construct_id": "C11A",
    ...
    },
    {
        ...
        "label": "International transfer readiness",
        "description": "The country shows signs that it is putting these procedures into practice, such as approving projects or preparing credits for international transfer.",
        "construct_id": "C11B",
    ...
    },
    {
        ...
        "description": "The country has social-protection systems – such as unemployment support, pensions, healthcare or disability support – that can protect people affected by economic disruption.",
        "construct_id": "C19",
...
.st-key-nav_validation_active .stButton > button {
...
.survey-privacy { color: var(--ink-muted); font-size: 12px; line-height: 1.6; margin: 12px 0 8px 0; }
.survey-details-gap { height: 50px; }
.priority-dropdown-gap { height: 35px; }
.survey-question-intro { font-size: 17px; font-weight: 600; line-height: 1.45; margin: 0 0 8px 0; color: var(--ink); }
.survey-scenario-copy { color: var(--ink-muted); font-size: 13px; line-height: 1.62; margin: 0 0 22px 0; }
...
.st-key-survey_submit_row .stButton > button { width: 100%; }
.st-key-validation_section [data-testid="stPopover"] button {
    border: 0 !important;
    background: transparent !important;
    color: var(--accent) !important;
...
.st-key-validation_section [data-testid="stPopover"] button:hover { color: var(--accent) !important; opacity: .78; }
@media (max-width: 900px) {
    .st-key-validation_section { padding: 20px 18px; }
    .st-key-validation_form_shell { padding: 8px 2px 30px 2px; }
}
...
def render_sidebar(view: str) -> None:
    with st.sidebar:
        ...
        if st.button("Research Survey", key="sidebar_validation", type="tertiary"):
            navigate_to("validation", scroll_target="top")
...
def render_top_navigation(view: str, market_period: str | None = None) -> None:
    with st.container(key="top_nav"):
        nav_columns = st.columns([1.2, 1.68, 2.05, 1.75, 2.32], gap="small")
        nav_items = [
            ("overall", "Main Index"),
            ("market", "Market Confidence"),
            ("priorities", "Add Your Priorities"),
            ("validation", "Research Survey"),
        ]
        for column, (target, label) in zip(nav_columns[:4], nav_items):
...
def validation_more_info() -> None:
    with st.popover("More about coal-to-clean projects and transition credits"):
        st.markdown(
            """
            Coal-to-clean projects seek to close a coal-fired power plant earlier than it would otherwise be expected to retire.

            Project finance may support early closure and decommissioning, new renewable electricity generation, grid-related measures, and support for affected workers and communities.

            The emissions avoided through early closure are compared with a baseline representing how long the plant would otherwise have operated. Verified net emission reductions before the baseline retirement date may be issued as carbon credits, sometimes called transition credits.

            Verra and Gold Standard have published methodologies for this project type. Several proposed transactions are being assessed or piloted, but the market remains at an early stage.

            <a href="https://verra.org/methodologies/vm0052-accelerated-retirement-of-coal-fired-power-plants-using-a-just-transition-v1-0/" target="_blank" rel="noopener noreferrer">Verra VM0052 methodology</a><br>
            <a href="https://globalgoals.goldstandard.org/459_paa-m400-05_just-coal-decommissioning/" target="_blank" rel="noopener noreferrer">Gold Standard JUST: Coal Decommissioning methodology</a>
            """,
            unsafe_allow_html=True,
        )
...
def render_validation(index_data: pd.DataFrame) -> None:
    ...
    industry_options = ["Select your industry", *industries, "Other"]
    role_options = ["Select your role", *ROLE_OPTIONS]
    market_options = ["Global or multi-country role", *SURVEY_COUNTRIES, "Other"]
    host_countries = sorted(index_data["Country"].dropna().astype(str).unique(), key=str.casefold)

    with st.container(key="validation_section"):
        render_top_navigation("validation")
        st.markdown(
        ...
        st.markdown(
            ...
            <div class="survey-intro">
              ...
              <p>This survey examines which features of a host country – including its government, energy system, carbon market, financing conditions and social protections – most affect whether carbon-credit buyers and other market professionals would support such projects there.</p>
        ...
        if st.session_state.validation_submitted:
            st.success(st.session_state.validation_submission_message or "Thank you. Your responses have been recorded.")
            return
        ...
        with st.container(key="validation_form_shell"):
            ...
            st.markdown(
                """
                <div class="survey-privacy">Your details will be kept confidential and are requested solely for research purposes – to categorise respondent roles and, where you consent, to follow up about your responses.</div>
                """,
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                """
                <div class="survey-privacy">Your details will not be disclosed outside the research, except through services used to securely store responses. Findings will be reported only in aggregate or de-identified form.</div>
                """,
                unsafe_allow_html=True,
            )
            ...
            with col1:
                work_base = st.selectbox(
                    ...
                    ["Select a country", *SURVEY_COUNTRIES],
            ...
            intro_col, info_col = st.columns([0.9, 0.1])
            with intro_col:
                st.markdown(
            ...
            st.markdown(
                """
                <div class="survey-scenario-copy">Imagine that you are assessing whether to support a coal-to-clean transition-credit project. Support could involve buying credits through an offtake agreement, investing, financing, developing the project or recommending it professionally. Please answer from your professional perspective, even if you do not personally purchase carbon credits or work directly with carbon projects.</div>
                """,
                unsafe_allow_html=True,
            )
            st.markdown(
                '<div class="priority-scale-note"><span>1 = Not important</span><span>5 = Critical</span></div>',
                unsafe_allow_html=True,
            )
            ...
            for item in VALIDATION_FACTORS:
                st.markdown(
                    f'<div class="priority-label">{html.escape(item["label"])}</div>',
                    unsafe_allow_html=True,
                )
                st.markdown(
                    f'<div class="priority-description">{html.escape(item["description"])}</div>',
                ...
                factor_responses[item["key"]] = st.slider(
                    item["label"],
                    min_value=1,
                    max_value=5,
                    value=3,
                    step=1,
                    key=f'validation_factor_{item["key"]}',
                    label_visibility="collapsed",
                )

            st.markdown('<div class="survey-subtitle survey-later-question">Project’s host country</div>', unsafe_allow_html=True)
            project_setting = st.radio(
                "All else being equal, where would you be more inclined to support a coal-to-clean transition-credit project?",
                [
                    "Higher-income developed countries",
                    "Emerging and developing economies",
                    "No general preference",
                    "It depends on the particular project and country",
            ...
            st.markdown('<div class="survey-subtitle">Carbon-credit price for a coal-to-clean project</div>', unsafe_allow_html=True)
            ...
            st.markdown('<div class="survey-subtitle">Preferred host countries</div>', unsafe_allow_html=True)
            preferred_host_options = [
                *host_countries,
                "No particular country preference",
                "I do not have enough information to assess this",
            ]
            preferred_hosts = st.multiselect(
                "All else being equal, which host countries would you be most willing to support through a five-year commercial commitment?",
                preferred_host_options,
                max_selections=5,
                key="validation_preferred_hosts",
                help="Support could involve purchasing credits, investing, financing, developing the project or recommending it professionally. Select up to five.",
            )

            st.markdown('<div class="survey-subtitle">Coal phase-out initiatives</div>', unsafe_allow_html=True)
            st.markdown(
            ...
            st.markdown('<div class="survey-subtitle">Five most important factors</div>', unsafe_allow_html=True)
            top_five = st.multiselect(
                "Which five factors would have the greatest influence on your willingness to support a coal-to-clean transition-credit project?",
                [item["label"] for item in VALIDATION_FACTORS],
                max_selections=5,
                key="validation_top_five",
            )
            st.markdown(
            ...
            st.markdown('<div class="survey-subtitle">Anything missing?</div>', unsafe_allow_html=True)
            missing_factor = st.text_area(
                "Is there anything else about a country, its government or its market conditions that would make you more or less willing to support a coal-to-clean transition-credit project there?",
                key="validation_missing_factor",
                help="Optional",
            )

            with st.container(key="survey_submit_row"):
                submitted = st.button("Submit research survey", type="primary", key="validation_submit")
            ...
            if not organisation.strip():
                errors.append("Enter your organisation name.")
            if industry == "Select your industry":
                errors.append("Select your industry.")
            if industry == "Other" and not industry_other.strip():
                errors.append("Specify your industry or sector.")
            if role == "Select your role":
                errors.append("Select your role.")
            if role == "Other" and not role_other.strip():
                errors.append("Specify your role.")
            if work_base == "Select a country":
                errors.append("Select the country where you are currently based for work.")
            if not main_markets:
                errors.append("Select at least one main market of experience.")
            if "Other" in main_markets and not markets_other.strip():
                errors.append("Specify the other market or markets.")
            if contact_consent and not (email.strip() or linkedin.strip()):
                errors.append("Provide an email address or LinkedIn profile if you consent to follow-up.")
            if project_setting is None:
                errors.append("Answer the project host-country question.")
            if price_response is None:
                errors.append("Answer the carbon-credit price question.")
            if not preferred_hosts:
                errors.append("Select at least one preferred-host-country response.")
            exclusive_host_answers = {
                "No particular country preference",
                "I do not have enough information to assess this",
            }
            if any(value in preferred_hosts for value in exclusive_host_answers) and len(preferred_hosts) > 1:
                errors.append("Choose either named host countries or one of the no-preference options, not both.")
            initiative_choices = [value for value in [initiative_1, initiative_2, initiative_3] if value != "No selection"]
            if len(initiative_choices) != len(set(initiative_choices)):
                errors.append("Do not rank the same coal phase-out initiative more than once.")
            unsure_initiative = "I do not know enough about these initiatives to rank them"
            if unsure_initiative in initiative_choices and len(initiative_choices) > 1:
                errors.append("Select the insufficient-information option on its own.")
            if "Other" in initiative_choices and not initiative_other.strip():
                errors.append("Specify the other coal phase-out initiative.")
            if len(top_five) != 5:
                errors.append("Select exactly five most important factors.")
            ...
            display_order = 1
            for item in VALIDATION_FACTORS:
                response_value = factor_responses[item["key"]]
            ...
            response_rows.append(
                survey_response_row(
                    ...
                    question="All else being equal, where would you be more inclined to support a coal-to-clean transition-credit project?",
                    option_code=project_setting,
                    response_label=project_setting,
                    option_values="Higher-income developed countries | Emerging and developing economies | No general preference | It depends on the particular project and country | I do not have enough information to assess this",
                )
            )
            display_order += 1
            response_rows.append(
            ...
            for host in preferred_hosts:
                response_rows.append(
                    survey_response_row(
                        ...
                        question="Preferred host countries for a five-year commercial commitment",
                        option_code=host,
                        response_label=host,
                    )
                )
            ...
            if missing_factor.strip():
                response_rows.append(
                    survey_response_row(
                        ...
                        question="Anything else about a country, its government or market conditions that affects willingness to support a project",
                        response_text=missing_factor.strip(),
                    )
                )

            with st.spinner("Submitting your responses...", show_time=False):
            ...
            st.session_state.validation_submission_message = "Thank you. Your research-survey responses have been recorded."
            st.rerun()
