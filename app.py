...
/* Survey importance scales: endpoint labels sit at the container edges and the slider fills the centre. */
...
[class*="st-key-validation_factor_scale_"] [data-testid="stHorizontalBlock"] {
    align-items: center !important;
    flex-direction: row !important;
    gap: 14px !important;
}
[class*="st-key-validation_factor_scale_"] [data-testid="column"] {
...
.survey-scale-endpoint-left { text-align: left; }
.survey-scale-endpoint-right { text-align: right; }
...
@media (max-width: 900px) {
    ...
    /* Keep each importance scale in one horizontal row on mobile. */
    .st-key-validation_form_shell [class*="st-key-validation_factor_scale_"] [data-testid="stHorizontalBlock"] {
        flex-direction: row !important;
        align-items: center !important;
        gap: 7px !important;
    }
    .st-key-validation_form_shell [class*="st-key-validation_factor_scale_"] [data-testid="column"] {
        min-width: 0 !important;
    }
    .st-key-validation_form_shell [class*="st-key-validation_factor_scale_"] [data-testid="column"]:nth-child(1) {
        flex: 0 0 88px !important;
        width: 88px !important;
    }
    .st-key-validation_form_shell [class*="st-key-validation_factor_scale_"] [data-testid="column"]:nth-child(2) {
        flex: 1 1 auto !important;
        width: auto !important;
    }
    .st-key-validation_form_shell [class*="st-key-validation_factor_scale_"] [data-testid="column"]:nth-child(3) {
        ...
        with st.container(key="validation_form_shell"):
            ...
            for factor_number, item in enumerate(VALIDATION_FACTORS, start=1):
                ...
                with st.container(key=f'validation_factor_scale_{item["key"]}'):
                    scale_left, scale_slider, scale_right = st.columns([1.55, 7.2, 1.25], gap="small")
                    with scale_left:
                        st.markdown(
                            '<div class="survey-scale-endpoint survey-scale-endpoint-left">1 = Not important</div>',
                            unsafe_allow_html=True,
                        )
                    with scale_slider:
                    ...
                    with scale_right:
                        st.markdown(
                            '<div class="survey-scale-endpoint survey-scale-endpoint-right">5 = Critical</div>',
                            unsafe_allow_html=True,
                        )
            ...
            if not initiative_choices:
                errors.append(
                    "Q26 – Coal phase-out initiatives: rank at least one initiative, or select the option stating that you do not know enough to rank them."
                )
            if len(initiative_choices) != len(set(initiative_choices)):
                errors.append("Q26 – Coal phase-out initiatives: do not rank the same initiative more than once.")
            ...
            if unsure_initiative in initiative_choices and len(initiative_choices) > 1:
                errors.append(
                    "Q26 – Coal phase-out initiatives: select the insufficient-information option on its own."
                )
            if "Other" in initiative_choices and not initiative_other.strip():
                errors.append("Q26 – Coal phase-out initiatives: specify the other initiative.")
            if len(top_five) != 5:
            ...
            st.session_state.validation_submission_message = "Thank you. Your responses have been recorded."
...
if survey_route_active and not st.session_state.validation_submitted:
    st.session_state.survey_entry_locked = True

survey_mode_active = bool(
    survey_route_active
    or (
        st.session_state.survey_entry_locked
        and not st.session_state.validation_submitted
    )
)
...
if st.session_state.survey_success_pending and view == "overall":
    st.toast("Thank you. Your responses have been recorded.")
